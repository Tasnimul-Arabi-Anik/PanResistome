...
11. ABRicate NCBI/VFDB/PlasmidFinder are available after setup
    ...
    - Note: `--panr2_update_abricate_db true` is the default and force-refreshes requested ABRicate databases with `abricate-get_db --force` when the ABRicate helper is available. Use `--panr2_update_abricate_db false` for offline/frozen reruns.
