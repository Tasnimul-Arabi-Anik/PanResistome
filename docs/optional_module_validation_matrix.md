...
| geNomad/prophage | Experimental runner with Docker DB-download validation and host-env bypass | Stable table input | geNomad DB, downloaded when enabled and no DB path is supplied, or supplied with `--genomad_db` | Synthetic PanR2 contract test; broad optional-module table validation; optional-runner smoke without DB; Docker geNomad DB v1.9 download passed; two-genome Docker geNomad run passed with clean feature-contract validation | `prophage.features.tsv` | No | Docker validates database download, mounted DB reuse, runner execution, and PanR2 contract export. The two-genome Klebsiella run produced a header-only prophage table, so positive biological prophage calls still need a prophage-rich validation dataset. Use `-profile genomad_host` or Docker with `--genomad_use_host_env true` to bypass only the geNomad Conda solve. |
