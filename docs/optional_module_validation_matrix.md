...
| geNomad/prophage | Experimental runner with Docker DB-download, host-env bypass, and one-genome positive-call validation | Stable table input | geNomad DB, downloaded when enabled and no DB path is supplied, or supplied with `--genomad_db` | Synthetic PanR2 contract test; broad optional-module table validation; optional-runner smoke without DB; Docker geNomad DB v1.9 download passed; two-genome Docker geNomad run passed; one-genome Docker/GHCR positive-call validation passed | `prophage.features.tsv` | No | Docker validates database download, mounted DB reuse, runner execution, and PanR2 contract export. The positive-call run required `--genomad_splits 8 --genomad_sensitivity 3.0` after default MMseqs searches were killed by memory pressure. Keep opt-in until 2-10 genome and larger scalability validations pass. Use `-profile genomad_host` or Docker with `--genomad_use_host_env true` to bypass only the geNomad Conda solve. |
...
3. **geNomad scale validation:** expand the one-genome positive-call Docker/GHCR validation to 2-10 genomes with `--genomad_splits` tuned for memory.
