...
| geNomad/prophage | Experimental runner with auto-download helper and host-env bypass | Stable table input | geNomad DB, downloaded when enabled and no DB path is supplied | Synthetic PanR2 contract test; broad optional-module table validation; optional-runner smoke without DB; 5-genome auto-download attempt reached environment creation but did not reach DB download; `genomad_host` negative smoke passed | `prophage.features.tsv` | No | PanResistome supports `--genomad_auto_download_db true` into `--genomad_db_dir`, but full biological runner validation still needs a local/fresh-downloaded DB. First-run Conda environment creation was the observed bottleneck. Use `-profile genomad_host` with a prebuilt host/container `genomad` executable to bypass only the geNomad Conda solve. |
