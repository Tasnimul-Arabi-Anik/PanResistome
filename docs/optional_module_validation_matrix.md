...
| MOB-suite | Biologically validated small-run runner | Stable table input | MOB-suite environment/database plus ETE `taxa.sqlite` | Synthetic PanR2 contract test; optional-runner smoke; Klebsiella 2-genome biological validation | `mobsuite.features.tsv` | No | The repo env now installs `mob-suite` through pip to avoid Bioconda post-link fragility. If `--mobsuite_db` is omitted, PanResistome now uses `--mobsuite_db_dir` and runs audited `mob_init`/taxa setup helpers. Fresh-machine cache initialization still needs validation. |
